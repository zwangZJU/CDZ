...
public class HttpApiRouterController {

	private static final Logger logger = LoggerFactory.getLogger(HttpApiRouterController.class);
	@Autowired
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			commonLogsPO.setContent(router+":1:"+e.getTargetException().getMessage());
			commonLogsPO.setOper_id("HttpApiRouterController:InvocationTargetException");
			commonLogsPO.setOper_name("HttpApiRouterController");
			commonLogsDao.insert(commonLogsPO);
			e.printStackTrace();
			//logger.info("HttpApiRouterController:Exception::::{}",e.getTargetException().getMessage());
			...
			commonLogsPO.setCreate_date(AOSUtils.getDateTime());
			commonLogsPO.setIs_del("0");
			commonLogsPO.setContent(router+":2:"+e.getMessage());
			commonLogsPO.setOper_id("HttpApiRouterController:Exception");
			commonLogsPO.setOper_name("HttpApiRouterController");
			commonLogsDao.insert(commonLogsPO);
	...
	public String wxNotify(HttpServletRequest request, HttpServletResponse response) {
		HttpModel httpModel = new HttpModel(request, response);
		String router = "appApiService.wxNotify";
	...
	public String aliNotify(HttpServletRequest request, HttpServletResponse response) {
		HttpModel httpModel = new HttpModel(request, response);
		String router = "appApiService.aliNotify";
		router = AOSUtils.isEmpty(router) ? httpModel.getInDto().getString("router") : router;
		try {
			if (AOSUtils.isEmpty(router) || StringUtils.lastIndexOf(router, ".") == StringUtils.INDEX_NOT_FOUND) {
				throw new AOSException("请求路由规则参数缺失或路由规则不合法，请检查。" );
