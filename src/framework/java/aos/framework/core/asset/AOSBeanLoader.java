...
import aos.framework.web.WebApplicationContextExporterListener;
...
public class AOSBeanLoader {
	...
	private static Logger log = LoggerFactory.getLogger(AOSBeanLoader.class);
		...
		if (applicationContext == null) {
			try {
				if (log.isInfoEnabled())
					log.info("系统正在初始化...");
				applicationContext = new ClassPathXmlApplicationContext(new String[] { "spring-mybatis.xml" });
				if (log.isInfoEnabled())
					log.info("初始化完成.");
			} catch (Exception e) {
