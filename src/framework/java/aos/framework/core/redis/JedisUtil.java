...
public class JedisUtil {
	...
	static {
        ...
        redis_password = AOSPropertiesHandler.getProperty("redis_password");
		...
		jedisPool = new JedisPool(config, host, port,60000,redis_password);
	}
	...
	public static Jedis getJedisClient() {
		...
		try {
			...
			logger.error("获取Redis客户端连接失败。"+redis_password);
			e.printStackTrace();
		}
		if (jedis == null) {
			logger.warn("没有获取到Redis客户端连接。"+redis_password);
		}
		return jedis;
	}
