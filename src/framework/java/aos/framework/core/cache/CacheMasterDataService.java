 ...
 */
@Service
public class CacheMasterDataService extends CDZBaseController{
	...
	public void cacheSMSCode(String sessionId,String code){
		Jedis jedis = JedisUtil.getJedisClient();
		jedis.set(sessionId, code, "NX", "EX", 60*5);//有效时间300秒
		JedisUtil.close(jedis);
	}
	
	public String getSMSCode(String sessionId){
		String desc = StringUtils.EMPTY;
		Jedis jedis = JedisUtil.getJedisClient();
		desc=jedis.get(sessionId);
		JedisUtil.close(jedis);
		return desc;
	}
