...
public class CacheUserDataService {
	...
	public UserModel getUserModel(String juid) {
		...
		if (AOSUtils.isNotEmpty(juid)) {
			...
			if(AOSUtils.isNotEmpty(userJson)){
	 ...
	 * @param juid
	 */
	public void logoutDealer(String juid){
		//重置缓存用户信息
		resetUserModel(juid);
	...
	public void heartbeatDealer(String juid){
		JedisUtil.exprString(juid, Integer.valueOf(AOSCxt.getParam("dealer_login_timeout_")));
	}
