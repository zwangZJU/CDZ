...
<body>
 ...
 function test(date_start,date_end){
	 	var cosPoints = []; 
	    var cosPoints1 = [];
			$.post("${ctx}/http/do.jhtml?router=tongJiController.listAdvertTrafficsStatFigure&juid=${juid}",
		  {
		  ...
		  function(datas,status){
			  ...
			  for(i=0;i<datas.length;i++){
				  var data=datas[i];
				  cosPoints1.push([data.create_date, data.num]);
			  }
	...
	if(lineMemory==null){
		lineMemory=$.jqplot('UserChart',
				array,
	            {
	                //图标标题
	                title: {
	                    text: date_start+'至'+date_end+'广告展示次数',
	                    show: true,
	                ...
	                series: [{
	                    label: '广告展示次数',//按data先后顺序显示每种分类名称
	                    markerOptions:
	                        {
	                            size: 6,
	                            style: 'circle'
