...
<body>
 ...
 function test(date_start,date_end){
	 	var cosPoints = []; 
	    var cosPoints1 = [];
			$.post("${ctx}/http/do.jhtml?router=tongJiController.chargingOrdersSalesStatFigure&juid=${juid}",
		  {
		  ...
		  function(datas,status){
			  ...
			  for(i=0;i<datas.length;i++){
				  var data=datas[i];
				  cosPoints1.push([data.create_date, data.total_amt]);
			  }
	...
	if(lineMemory==null){
		lineMemory=$.jqplot('UserChart',
				array,
	            {
	                //图标标题
	                title: {
	                    text: date_start+'至'+date_end+'销售统计',
	                    show: true,
